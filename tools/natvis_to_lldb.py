...
def generate_lldb_formatter(
    types: list[NatvisType],
    *,
    category: str,
    source_name: str,
    enable_expression_eval: bool = False,
    enable_summaries: bool = True,
    enable_synthetic_children: bool = True,
) -> str:
...
ENABLE_EXPRESSION_EVAL = __ENABLE_EXPRESSION_EVAL__
...
def __lldb_init_module(debugger, internal_dict):
    ...
    if not ENABLE_SUMMARIES and not ENABLE_SYNTHETIC_CHILDREN:
        print("Loaded Natvis debug commands only. Formatters were not registered.")
        return

    debugger.HandleCommand("type category define " + _quote_lldb(CATEGORY))
    for spec in FORMATTERS:
        regex = _quote_lldb(spec["regex"])
        if ENABLE_SUMMARIES and (spec.get("display_string") or spec.get("string_view")):
        ...
        if ENABLE_SYNTHETIC_CHILDREN and (
            spec.get("items") or spec.get("array_items") or spec.get("custom_list_items")
        ):
            debugger.HandleCommand(
...
"""
    return textwrap.dedent(generated).replace(
        ...
        "__ENABLE_EXPRESSION_EVAL__", repr(enable_expression_eval)
    ).replace(
        "__ENABLE_SUMMARIES__", repr(enable_summaries)
    ).replace(
        "__ENABLE_SYNTHETIC_CHILDREN__", repr(enable_synthetic_children)
    )
...
def write_formatter(
    ...
    *,
    ...
    output_path.write_text(
        generate_lldb_formatter(
            result.types,
            category=category,
            source_name=input_path.name,
            enable_expression_eval=enable_expression_eval,
            enable_summaries=enable_summaries,
            enable_synthetic_children=enable_synthetic_children,
        ),
        encoding="utf-8",
    )
    return result
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Allow generated formatters to call LLDB EvaluateExpression as a fallback. "
            "Disabled by default because it can freeze CodeLLDB while variables render."
        ),
    )
    parser.add_argument(
        "--no-synthetic",
        action="store_true",
        help="Register summaries and debug commands, but do not register synthetic children.",
    )
    parser.add_argument(
        "--commands-only",
        action="store_true",
        help="Only register natvis-debug commands. Do not install type summaries or synthetic providers.",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = write_formatter(
        args.natvis,
        args.output,
        args.category,
        enable_expression_eval=args.enable_expression_eval,
        enable_summaries=not args.commands_only,
        enable_synthetic_children=not args.commands_only and not args.no_synthetic,
    )
